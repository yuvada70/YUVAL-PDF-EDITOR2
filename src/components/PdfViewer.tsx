...
export function PdfViewer() {
  ...
  // currentPage is a position in pageOrder; resolve to original index for rendering
  const origIdx = store.pageOrder[store.currentPage] ?? 0
  ...
  }, [store.pdfFile, origIdx, store.zoom, rotation])
  ...
  return (
    <div ref={containerRef} className="flex-1 overflow-auto bg-slate-400 flex justify-center py-6">
      {loading && (
      ...
      <div
        ...
        onMouseUp={() => { highlightDragRef.current = null }}
        onClick={handleCanvasClick}
      >
        <canvas ref={canvasRef} className="pdf-page-canvas" />
        {canvasSize.width > 0 && (
          <>
            <AnnotationLayer canvasWidth={canvasSize.width} canvasHeight={canvasSize.height} />
            {store.tool === 'draw' && (
              <DrawLayer canvasWidth={canvasSize.width} canvasHeight={canvasSize.height} />
            )}
          </>
        )}
      </div>
